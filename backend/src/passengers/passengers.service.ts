    ...
    private passengerRepository: Repository<Passenger>,
        ...
        async findAll(): Promise<any> {
                ...
                async create(createPassengerDto: CreatePassengerDto) {
                    const booking = await this.bookingRepository.findOne({ where:{bookingId:createPassengerDto.bookingId} });

                    if (!booking) {
                        throw new NotFoundException(`Aircraft with ID ${createPassengerDto.bookingId} not found`);
                    }
